...
JMM：不正确同步程序，不保证与顺序一致性模型一致，并且每个线程看到的顺序也可是不一致的。因为JMM要保持一致需要禁止大量的重排序，再因为总体上程序也是无序的，这样做没有意义。但是JMM提供最小安全性，即确保要么是之前的值，要么是默认值（JMM在创建对象的时候会对内存区域进行清零，然后存入值）<br>对于正确同步的程序，JVM会在结果一致的情况下，尽量让编译器和处理器进行重排序（临界区内的代码，可能允许重排序）
